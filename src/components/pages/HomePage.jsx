...
function HomePage() {
  ...
  // Mobile configuration - simpler, single card display
  const mobileCardWidth = 280; // Slightly smaller for mobile
  const mobileCarouselHeight = 360;

  return (
    <div>
      <Hero />

      <section id="categories" className="py-12 md:py-20 relative overflow-hidden bg-transparent dark:bg-transparent">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <motion.h2
              className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 bg-gradient-to-r from-gray-900 via-primary to-blue-600 dark:from-gray-100 dark:via-blue-400 dark:to-blue-500 bg-clip-text text-transparent px-4"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
        ...
        <div
          ...
          // Use calculated height - responsive for mobile
          style={{ 
            height: isMobile ? `${mobileCarouselHeight}px` : `${carouselHeight}px`, 
        ...
        >
           ...
           <div className="relative w-full h-full" style={{ transformStyle: 'preserve-3d' }}>
             <AnimatePresence initial={false}>
              {categories.map((category, index) => {
                ...
                // Mobile: show only current card, hide others
                if (isMobile && absOffset > 0) return null;
                
                // Desktop 3D carousel logic
                const activeCardWidth = isMobile ? mobileCardWidth : cardWidth;
                const x = isMobile ? 0 : offset * (cardWidth / 2 + gap);
                const scale = isMobile ? 1 : Math.pow(scaleFactor, absOffset);
                const rotateY = isMobile ? 0 : offset * rotationAngle * -1;
                const zIndex = numCategories - absOffset;
                const opacity = isMobile ? 1 : (absOffset > 2 ? 0 : 0.9);
                const translateZ = isMobile ? 0 : -absOffset * zOffsetFactor;
                ...
                return (
                  <motion.div
                    ...
                    className="absolute top-1/2 left-1/2 cursor-pointer" // Origin still center-center
                     ...
                     style={{
                       ...
                       height: isMobile 
                         ? `${mobileCardWidth}px` 
                         : `${cardWidth + reflectionGap + (cardWidth * reflectionHeightRatio)}px`,
                    ...
                    initial={{
                       opacity: 0, x: "-50%",
                       translateX: isMobile ? 0 : x * 2, 
                       scale: isMobile ? 0.8 : 0.5,
                       rotateY: isMobile ? 0 : rotateY + (offset > 0 ? 30 : -30),
                       translateZ: isMobile ? 0 : translateZ - 100,
                    }}
                    animate={{
                    ...
                    exit={{
                      opacity: 0, 
                      translateX: isMobile ? (offset > 0 ? 100 : -100) : x * (offset > 0 ? 1.5 : -1.5), 
                      scale: isMobile ? 0.8 : 0.3,
                      rotateY: isMobile ? 0 : rotateY + (offset > 0 ? 45 : -45), 
                      translateZ: isMobile ? 0 : translateZ - 150,
                    }}
                  ...
                  >
                    ...
                    <Link
                      ...
                      style={{ height: isMobile ? `${mobileCardWidth}px` : `${cardWidth}px` }}
          ...
          <motion.button
            ...
            className="absolute left-2 md:left-4 lg:left-10 top-1/2 transform -translate-y-1/2 z-20 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm text-primary dark:text-blue-400 p-3 md:p-4 rounded-full hover:bg-primary dark:hover:bg-blue-600 hover:text-white dark:hover:text-white hover:scale-110 transition-all duration-300 shadow-xl border-2 border-transparent dark:border-gray-700"
            aria-label="Previous Category"
            whileHover={{ scale: 1.1, x: -5 }}
            whileTap={{ scale: 0.95 }}
          >
            <FaChevronLeft size={isMobile ? 20 : 24} />
          </motion.button>
          <motion.button
            onClick={goToNext}
            className="absolute right-2 md:right-4 lg:right-10 top-1/2 transform -translate-y-1/2 z-20 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm text-primary dark:text-blue-400 p-3 md:p-4 rounded-full hover:bg-primary dark:hover:bg-blue-600 hover:text-white dark:hover:text-white hover:scale-110 transition-all duration-300 shadow-xl border-2 border-transparent dark:border-gray-700"
            aria-label="Next Category"
            whileHover={{ scale: 1.1, x: 5 }}
            whileTap={{ scale: 0.95 }}
          >
            <FaChevronRight size={isMobile ? 20 : 24} />
          </motion.button>
            ...
            {categories.map((_, index) => (
              <motion.button
                ...
                className={`transition-all duration-300 rounded-full ${
                  index === currentIndex 
                    ? 'bg-primary w-8 md:w-12 h-2 md:h-3' 
